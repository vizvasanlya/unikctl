...
INSTALL_SERVER=${INSTALL_SERVER:-https://get.unikctl.sh}
...
Documentation:    https://unikctl.sh/
...
install_linux_manual() {
    ...
    if check_cmd "unikctl"; then
        _ill_kraft_path="$($WHICH unikctl)"
        _ill_prompt_msg=$(printf "%s%s%s"                                       \
            "The unikctl binary already installed at \"$_ill_kraft_path\", "      \
...
install_darwin_manual() {
    ...
    # Check if binary already exists
    if check_cmd "unikctl"; then
        _idr_kraft_path="$($WHICH unikctl)"
        _idr_prompt_msg=$(printf "%s%s%s"                                       \
            "The unikctl binary already installed at \"$_idr_kraft_path\", "      \
...
install_completions() {
    ...
    do_cmd "unikctl completion $_inc_shell > $_inc_kraft_config_file"
...
collect_telemetry() {
    ...
    if prompt_yes_no "Help us make KraftKit better and opt-in to collecting anonymous telemetry? [y/N]: " "n"; then
        ...
        _et_telemetry="n"
        unikctl system set collect_anonymous_telemetry=false
    fi
